...
void Essai2()
{
  ...
  c.setNom("Hamilton");
  Person *p = (Person *)&c;    // on fait quoi ici ? => upcasting
  p->affiche();
  cout << endl;
...
void Essai4()
{
  ...
  for (int i=0 ; i<10 ; i++)
  {
	...
	FederationAdmin* pFedAdm = dynamic_cast<FederationAdmin*>(person[i]);
	if(pFedAdm != NULL) cout << "est un FederationAdmin" << endl;
	TeamMember* pTeaMem = dynamic_cast<TeamMember*>(person[i]);
	if(pTeaMem != NULL) cout << "est un TeamMember" << endl;
